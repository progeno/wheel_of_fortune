...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        res = [s for s in LETTERS if s not in guessed and (self.prizeMoney >= VOWEL_COST or s not in VOWELS)]
        return res
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if self.smartCoinFlip:
            i = -1
            while WOFComputerPlayer.SORTED_FREQUENCIES[i] not in possibleLetters:
                i -= 1
            return WOFComputerPlayer.SORTED_FREQUENCIES[i]
        else:
